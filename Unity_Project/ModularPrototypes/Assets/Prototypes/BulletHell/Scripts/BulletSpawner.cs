...
namespace ModularPrototypes.BulletHell
{
    public class BulletSpawner : MonoBehaviour
    {
        ...
        [SerializeField] private float _invokeDuration = 2f;
        ...
        private void Start()
        {
            ...
            InvokeRepeating(nameof(Fire), 0f, _invokeDuration);
        }
        ...
        private void RadialBurst()
        {
            ...
            for (int i = 0; i < _bulletsAmount + 1; i++)
            {
                ...
                bullet.GetComponent<Bullet>().SetProperties(bulletDirection, _radialBurstMaterial, _bulletSpeed);
        ...
        private void Spiral()
        {
            ...
            bullet.transform.SetPositionAndRotation(transform.position, transform.rotation);
            bullet.SetActive(true);
            bullet.GetComponent<Bullet>().SetProperties(bulletDirection, _spiralMaterial, _bulletSpeed);
        ...
        private void DoubleSpiral()
        {
            for (int i = 0; i <= 1; i++)
            {
                ...
                bullet.transform.SetPositionAndRotation(transform.position, transform.rotation);
                bullet.SetActive(true);
                bullet.GetComponent<Bullet>().SetProperties(bulletDirection, _doubleSpiralMaterial, _bulletSpeed);
            }
